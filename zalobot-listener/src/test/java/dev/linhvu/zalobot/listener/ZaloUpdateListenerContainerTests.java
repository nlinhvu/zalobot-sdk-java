...
class ZaloUpdateListenerContainerTests {
	...
	@Test
	void pause_stopsInvokingListener() throws InterruptedException {
		ZaloApiResponse<GetUpdatesResult> response = new ZaloApiResponse<>(true, sampleUpdate(), 0);
		ZaloBotClient client = mockClient(response);
		ContainerProperties props = fastProperties();
		...
		int countAtPause = counter.get();
		Thread.sleep(200);
		// Counter should not have increased after pause
		assertThat(counter.get()).isLessThanOrEqualTo(countAtPause + 1);
		container.stop();
	}

	@Test
	void resume_afterPause_resumesInvokingListener() throws InterruptedException {
		ZaloApiResponse<GetUpdatesResult> response = new ZaloApiResponse<>(true, sampleUpdate(), 0);
		ZaloBotClient client = mockClient(response);
		ContainerProperties props = fastProperties();
		...
		try {
			assertThat(afterResume.await(5, TimeUnit.SECONDS)).isTrue();
		}
		finally {
			container.stop();
		}
	...
	@Test
	void isContainerPaused_reflectsActualConsumerState() throws InterruptedException {
		ZaloApiResponse<GetUpdatesResult> response = new ZaloApiResponse<>(true, sampleUpdate(), 0);
		ZaloBotClient client = mockClient(response);
		ContainerProperties props = fastProperties();
		...
		CountDownLatch firstPoll = new CountDownLatch(1);
		ZaloUpdateListenerContainer container = new ZaloUpdateListenerContainer(client, props);
		container.setUpdateListener(update -> firstPoll.countDown());

		container.start();
		assertThat(firstPoll.await(5, TimeUnit.SECONDS)).isTrue();

		container.pause();
		// Wait for the consumer to actually pause
